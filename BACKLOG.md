...
- [ ] **#22** Pre-publish: confirm the CrewLAB join link and donation collection are current `#improvement` `#ecxc` *(2026-06-04, re-scoped 2026-07-13)*
  Before beta launch, check against the live CrewLAB setup that the join deep link (`crewlab.app.link/5g7vhhYEn3b`) still resolves and that ECXC actually collects donations through the app. Re-scoped 2026-07-13: waiver signing and registration moved to the on-site forms (`/training`, `/talkeetna`), so the signing-flow checks dropped out; sweep `crewlab.md` and `about.md` for any remaining sign-in-CrewLAB claims while confirming.
