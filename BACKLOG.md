...
- [ ] **#22** Pre-publish: revoke the old CrewLAB join link; confirm donation collection `#improvement` `#ecxc` *(2026-06-04, re-scoped twice, last 2026-07-13)*
  The join-link half flipped meaning with the invite-only flow (2026-07-13): the public deep link (`crewlab.app.link/5g7vhhYEn3b`) is off the site, but it stays a live credential for anyone holding it, and rotation/revocation is undocumented (see `docs/crewlab-onboarding-research.md`). Ask CrewLAB support about revoking or regenerating it, or check the Invite Teammates screen for a regenerate option. Still owed from the original scope: confirm whether ECXC actually collects donations through the app (#21's answer feeds the same passage).
- [ ] **#21** Confirm what ECXC collects through CrewLAB, then add the answer to crewlab.md `#improvement` `#ecxc` *(2026-06-04, re-scoped 2026-07-13)*
  Decide what the program actually collects through the app (camp cost-share, dues, or donations), then add the answer to the "For parents & supporters" passage of `src/content/pages/crewlab.md`. Re-scoped 2026-07-13: the paragraph carrying the unconfirmed payments claim (and a raw `[ASK: ...]` marker that was rendering on the live page) was cut in the backlog-triage pass; the page now says nothing about payments until this is confirmed.
