<!-- @component
ECXC's Training page: the 'training' pages-concept entry's editorial content, rendered the
same way [...path]/+page.svelte renders any page entry, with the on-page registration form
below it. A bespoke route rather than the catch-all (see +page.server.ts), so this duplicates
the catch-all's page-entry markup and its `.page-title` accent-bar style rather than importing
that component: a route that never renders [...path]/+page.svelte does not ship its scoped CSS.
-->
<script lang="ts">
...
<div class="mt-2xl">
  <RegistrationForm variant="training" />
</div>
